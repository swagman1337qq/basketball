...
| New ratings: Acceleration (first step), Layups (split from Dunks), Boxing out (team rebounding without the individual numbers); measured wingspan per player (−6″ to +12″ vs height) that affects contests, blocks, rebounds and steals; new badges (Blow-By, Crafty Finisher, Box-Out Beast, Condor) | Done | `RATING_KEYS`, `sim.ts`, derived on load for old saves |
| God Mode: hometown (city + country) and pre-NBA team editable; generating a name from a country moves the hometown there; scouting reports label the scout | Done | `GodPlayerEditor` |
