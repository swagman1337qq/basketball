...
export function buildView(gm: Game, rootRef: RefObject<HTMLDivElement | null>, extra: ViewExtras): VM {
  ...
  const fav = mine.slice().sort((x, y) => P[y].pot - P[x].pot)[0], firsts = s.assets.filter(a => a.owner === s.me && a.rd === 1 && !usedPick(a)).length, bestOvr = Math.max(...mine.map(id => P[id].ovr)), full = att / 18800;
  ...
  else if (s.phase === 'preseason') { const overT = s.managed.filter(t => s.rosters[t].length > 15); ph.note = overT.length ? 'Cut to 15 players before opening night (' + overT.map(t => T[t].abbr + ' ' + s.rosters[t].length).join(', ') + ')' : 'Players have developed; ratings updated'; ph.actions = [{ ...act('Start the regular season', () => gm.startSeason(), true), dis: overT.length > 0 }]; }
