...
export function buildView(gm: Game, rootRef: RefObject<HTMLDivElement | null>, extra: ViewExtras): VM {
  ...
  const tone = v => v >= 65 ? 'var(--color-accent-700)' : v < 45 ? 'var(--color-neutral-500)' : 'var(--color-text)';
  ...
  const T = s.teams, me = T[0], mine = s.rosters[0], pct = t => gm.pct(t);
  ...
  const g = d.sched[s.day % d.sched.length], opp = T[g.opp];
  const next = { openT: openTeam(g.opp), when: gm.fmtS(s.day), vs: g.home ? 'vs' : 'at', abbr: opp.abbr, oppName: opp.region + ' ' + opp.name, oppRec: opp.w + '–' + opp.l, myRec: me.w + '–' + me.l, line: (g.home ? 'Home · Harbor Arena' : 'Road · ' + opp.region) + ' · ' + ord(T.filter(t => t.conf === opp.conf).sort(byPct).indexOf(opp) + 1) + ' in the ' + opp.conf };
  const results = s.results.slice(0, 6).map(r => ({ openT: openTeam(r.opp), date: gm.fmtS(r.day), opp: (r.home ? 'vs ' : 'at ') + T[r.opp].abbr, wl: r.win ? 'W' : 'L', color: r.win ? 'var(--color-accent-700)' : 'var(--color-neutral-600)', score: r.us + '–' + r.them }));
  ...
  const lineup = mine.slice(0, 5).map(id => ({ ...pBase(id), face: gm.faceEl(id, 0) }));
  const confMini = confT.slice(0, 10).map((t, i) => ({ seed: i + 1, name: t.region + ' ' + t.name, rec: t.w + '–' + t.l, gb: gb(t, confT[0]), bg: t.tid === 0 ? 'var(--color-accent-100)' : 'transparent', fw: t.tid === 0 ? 600 : 400, openT: openTeam(t.tid), line: i === 5 ? '1px solid var(--color-text)' : '1px solid var(--color-divider)' }));
  const dashStats = [
  ...
  const moveTo = (id, to) => gm.setState(st => { const o = st.rosters[0].filter(x => x !== id); o.splice(cl(to, 0, o.length), 0, id); return { rosters: { ...st.rosters, 0: o }, sort: { ...st.sort, roster: ['rk', 1] }, dragId: null, overId: null }; });
  ...
  const status = ptid === 0 ? 'mine' : ptid === -1 ? 'fa' : ptid === -2 ? 'abroad' : ptid > 0 ? 'other' : 'pro';
  ...
  const career = [];
  if (status !== 'pro') {
    const abbr = ptid >= 0 ? T[ptid].region + ' ' + T[ptid].name : 'Free agent';
    for (let k = Math.min(4, pp.age - 19); k >= 1; k--) {
      const yr = (gm.Y - 1) - k, pre = yr < pp.draft, o = pp.ovr - k * (pp.age - k < 27 ? 3 : -1), f = cl((o - 36) / 30, 0.12, 1) * (pre ? 1.15 : 1);
      career.push({ season: yr + '–' + String(yr + 1 - 2000).padStart(2, '0'), team: pre ? pp.from.team : abbr, lg: pre ? pp.from.lg : 'League', gp: pre ? (pp.from.lg === 'NCAA' ? 30 + (pp.id + k) % 8 : 28 + (pp.id + k) % 12) : 58 + ((pp.id * 7 + k * 13) % 24), min: (10 + Math.min(1, f) * 24).toFixed(1), pts: (Math.max(2, (o - 32) * 0.72) * f).toFixed(1), reb: (f * pp.r.reb / 7.5).toFixed(1), ast: (f * pp.r.pss / 9).toFixed(1), per: cl(15 + (o - 50) * 0.75, 3, 32).toFixed(1), fw: 400 });
    }
    if (pp.gp) career.push({ season: gm.seasonLbl(), team: abbr, lg: 'League', gp: pp.gp, min: pp.min.toFixed(1), pts: pp.pts.toFixed(1), reb: pp.reb.toFixed(1), ast: pp.ast.toFixed(1), per: pp.per.toFixed(1), fw: 600 });
  }
  ...
  const pl = { ...pp, flag: gm.flag(pp.rep), cname: C[pp.rep].n, tone: tone(pp.ovr), face: gm.faceEl(pp.id, ptid),
    ...
    teamLabel: ptid === -2 ? 'Overseas · ' + pp.abroad.club + ' (' + pp.abroad.lg + ')' : ptid >= 0 ? T[ptid].region + ' ' + T[ptid].name : ptid === -1 ? 'Free agent' : pk ? 'Drafted #' + pk.n + ' by ' + T[gm.owner2027(pk.orig, s.assets)].abbr : 'Class of ' + pp.cls + ' prospect',
  ...
  const standGroups = grs.map(gr => { const ts = gr.ts.slice().sort(byPct); return { label: gr.label, rows: ts.map((t, i) => ({ seed: i + 1, name: t.region + ' ' + t.name, w: t.w, l: t.l, pct: fpct(t), gb: gb(t, ts[0]), home: t.hw + '–' + t.hl, road: t.rw + '–' + t.rl, l10: l10(t), strk: strk(t), bg: t.tid === 0 ? 'var(--color-accent-100)' : 'transparent', fw: t.tid === 0 ? 600 : 400, openT: openTeam(t.tid), line: conf && i === 5 ? '1px solid var(--color-text)' : conf && i === 9 ? '1px dashed var(--color-neutral-500)' : '1px solid var(--color-divider)' })) }; });
  ...
  const myAssets = s.assets.filter(a => a.owner === 0 && !usedPick(a)), theirAssets = s.assets.filter(a => a.owner === s.tTid && !usedPick(a));
  ...
  const tr = { out: money(out), in: money(inc), after: money(after), sal: agg ? 'Above the 2nd apron: salaries can\u2019t be combined' : after <= gm.CAP ? 'Under the cap' : salOk ? 'Matches' : after <= gm.AP1 ? 'Take back at most 125% + $0.25M' : 'Above the 1st apron: take back at most 100%', salColor: salOk ? 'var(--color-text)' : 'var(--color-accent-800)', ros: rosOk ? 'OK' : 'Exceeds 15 players', rosColor: rosOk ? 'var(--color-text)' : 'var(--color-accent-800)',
    stratName: STRAT[tst][0], stratDesc: STRAT[tst][1],
  ...
  const teamOptions = T.slice(1).map(t => ({ value: t.tid, label: t.region + ' ' + t.name + ' · ' + STRAT[strat[t.tid]][0] }));
  ...
  const cur = s.phase === 'draft' ? s.picks[s.pi] : null, onClock = !!cur && gm.owner2027(cur.orig, s.assets) === 0, taken = new Set(s.picks.filter(x => x.pid).map(x => x.pid));
  const myPicks = s.picks.filter(x => gm.owner2027(x.orig, s.assets) === 0), myNext = myPicks.find(x => !x.pid);
  ...
  const dr = { isCurrent: isCur, isFuture: !isCur, hasAdvice: advice.length > 0, advice, noAdvice: !myNext,
    ...
    myFuture: s.assets.filter(a => a.owner === 0 && a.yr === s.dClass).map(a => ({ label: gm.pickLabel(a, T), proj: projTxt(a) })),
    status: cur ? 'Pick ' + cur.n + ' · ' + (onClock ? 'You are on the clock' : T[gm.owner2027(cur.orig, s.assets)].region + ' ' + T[gm.owner2027(cur.orig, s.assets)].name + ' on the clock') : 'First round complete',
    ...
    order: s.picks.map((x, i) => { const ow = gm.owner2027(x.orig, s.assets); return { n: x.n, openT: openTeam(ow), abbr: T[ow].abbr, who: x.pid ? P[x.pid].name : i === s.pi ? 'On the clock' : (ow !== x.orig ? 'via ' + T[x.orig].abbr : ''), fs: x.pid ? 'normal' : 'italic', bg: i === s.pi ? 'var(--color-accent-100)' : 'transparent', color: ow === 0 ? 'var(--color-accent-700)' : 'var(--color-text)', fw: ow === 0 ? 600 : 400 }; }) };
  ...
  const att = cl(Math.round(18800 - (b.Tickets - 110) * 55 / mk + (wp - .5) * 9000 + (b.Facilities - 14) * 80 + (mk - 1) * 3000), 9000, 18800);
  const tix = b.Tickets * att * 41 / 1e6;
  const repeater = (s.taxHist || []).slice(-4).filter(Boolean).length >= 3; let taxBill = 0; { let over = payroll - gm.TAX, rate = 1.5 + (repeater ? 1 : 0); while (over > 0) { taxBill += Math.min(5, over) * rate; over -= 5; rate += rate < 1.75 ? 0.25 : rate < 2.5 ? 0.75 : 0.5; } }
  const share = 25 * (mk - 0.95);
  const rev: any[] = [['Ticket sales', tix], ['National media rights', 152.0], ['Local media', 34.0 * Math.pow(mk, 1.5)], ['Sponsorship & naming', 48.0 * mk], ['Merchandise', 22 * mk * (0.8 + wp * 0.4)]];
  if (share < 0) rev.push(['Revenue sharing received', -share]);
  if (payroll <= gm.TAX) rev.push(['Tax distribution (est.)', 11.5]);
  const exp: any[] = [['Player payroll', payroll], ...(taxBill ? [[repeater ? 'Luxury tax (repeater rates)' : 'Luxury tax', taxBill]] : []), ['Arena & game operations', 55.0], ['Front office & staff', 25.0], ['Team travel', 9.0], ...(share > 0 ? [['Revenue sharing paid', share]] : []), ...(s.buyoutCash ? [['Overseas buyouts', s.buyoutCash]] : []), ['Coaching', b.Coaching], ['Health', b.Health], ['Facilities', b.Facilities], ['Scouting', b.Scouting]];
  const net = rev.reduce((a, r) => a + r[1], 0) - exp.reduce((a, r) => a + r[1], 0);
  const EFF = { Coaching: 'Player progression ' + sgn(rel('Coaching') * 25) + ' vs league average', Health: 'Injury recovery ' + sgn(rel('Health') * 30) + ' vs league average', Facilities: 'Attendance and hype ' + sgn(rel('Facilities') * 15) + ', builds over seasons', Scouting: 'Draft ratings ±' + Math.round(3 * scoutF) + ' this year, ±' + Math.round(13 * scoutF) + ' two classes out', Tickets: att.toLocaleString() + ' per game · ' + Math.round(att / 188) + '% of capacity' };
  ...
  const toLive = (tid, ids) => ({ tid, name: T[tid].region + ' ' + T[tid].name, abbr: T[tid].abbr, rec: T[tid].w + '–' + T[tid].l, players: ids.filter(id => !P[id].inj && !P[id].dev).map(id => ({ id, name: P[id].name, pos: P[id].pos, grp: P[id].grp, ovr: P[id].ovr, r: P[id].r, crowd: P[id].pers.crowd, clutch: P[id].pers.clutch, adj: P[id].adjust > 0, flag: gm.flag(P[id].rep) })) });
  const usL = toLive(0, mine), themL = toLive(g.opp, s.rosters[g.opp].slice().sort((x, y) => P[y].ovr - P[x].ovr));
  const live = { home: g.home ? usL : themL, away: g.home ? themL : usL, userSide: g.home ? 'home' : 'away', tactics: s.tactics, onPlayer: id => gm.setState({ pid: id, modal: true, ptab: 'overview', extYears: null, extAmt: null, extMsg: null }), onTeam: tid => gm.setState({ teamModal: tid }), onFinish: r => { gm.sim(1, r); gm.setState({ screen: 'dash' }); } };
  const schedRows = [...s.results.slice().reverse().map(r => ({ openT: openTeam(r.opp), date: gm.fmtS(r.day), opp: (r.home ? 'vs ' : 'at ') + T[r.opp].region + ' ' + T[r.opp].name, rec: T[r.opp].w + '–' + T[r.opp].l, res: (r.win ? 'W ' : 'L ') + r.us + '–' + r.them, resColor: r.win ? 'var(--color-accent-700)' : 'var(--color-neutral-600)', isNext: false, notNext: true, bg: 'transparent' })),
    ...Array.from({ length: 15 }, (_, k) => { const gg = d.sched[(s.day + k) % d.sched.length], o = T[gg.opp]; return { openT: openTeam(gg.opp), date: gm.fmtS(s.day + k), opp: (gg.home ? 'vs ' : 'at ') + o.region + ' ' + o.name, rec: o.w + '–' + o.l, res: '—', resColor: 'var(--color-neutral-600)', isNext: k === 0, notNext: k !== 0, bg: k === 0 ? 'var(--color-accent-100)' : 'transparent' }; })];
  const typeOf = t => /^Traded/.test(t) ? 'Trade' : /^Signed/.test(t) ? 'Signing' : /^Released/.test(t) ? 'Release' : 'Draft';
  const allTx = [...s.log.map(l => ({ day: l.day ?? s.day, type: typeOf(l.text), teams: 'BAL', text: l.text, mine: true })), ...s.lgLog.map(e => ({ ...e, mine: false }))].sort((x, y) => y.day - x.day);
  const txRows = allTx.filter(x => s.txFilter === 'All' || x.type === s.txFilter).map(x => ({ date: gm.fmtS(x.day), type: x.type, teams: x.teams, teamLinks: String(x.teams).split(' · ').map(ab => ({ abbr: ab, open: openTeam(T.findIndex(t => t.abbr === ab)) })), text: x.text, bg: x.mine ? 'var(--color-accent-100)' : 'transparent' }));
    ...
    rows: l.ids.map(id => ({ ...pBase(id), team: teamOf(id), openT: openTeam(tidOf[id] ?? -1), money: tidOf[id] === -1 ? 'Asking ' + money(P[id].ask) : P[id].cls && tidOf[id] === undefined ? '—' : money(P[id].amt), remove: e => { e.stopPropagation(); togList(l.id, id)(); } })) }));
  {
    ...
    const FIT = { Winning: top ? 'Would see Baltimore as a contender.' : 'Your record would be a hard sell.', Money: 'Will follow the biggest offer.', Fame: me.mkt >= 1.1 ? 'Baltimore\u2019s market appeals to him.' : 'Would prefer a bigger market than Baltimore.', Loyalty: 'Hard to pry away from his current team.', 'Playing time': 'Would want a clear path to starter minutes.' };
    ...
    pl.tabOverview = (s.ptab || 'overview') === 'overview'; pl.tabContract = s.ptab === 'contract'; pl.tabHistory = s.ptab === 'history'; pl.tabEdit = s.ptab === 'edit' && !!s.god; pl.nativeSep = pp.native ? '| ' + pp.native : '';
    {
      ...
      pl.ed = { name: pp.name, native: pp.native || '', setName: e => mut(p => p.name = e.target.value), setNative: e => mut(p => p.native = e.target.value),
        ...
        repOpts: Object.keys(C).sort((x, y) => C[x].n.localeCompare(C[y].n)).map(c => ({ v: c, label: C[c].n })), repV: pp.rep, setRep: e => mut(p => { p.rep = e.target.value; if (!p.elig.find(x => x.c === p.rep)) p.elig = [...p.elig, { c: p.rep, why: 'set in God Mode' }]; }),
        ...
        traits: [['alpha', 'Wants to lead'], ['touches', 'Wants the ball'], ['pro', 'Professional'], ['volatile', 'Volatile'], ['crowd', 'Crowd reliant'], ['clutch', 'Clutch'], ['prone', 'Injury prone']].map(([k, label]) => ({ label, ...chip(!!pp.pers[k]), toggle: () => mut(p => p.pers[k] = !p.pers[k]) })),
    ...
    var ptabs = [['overview', 'Overview'], ['contract', 'Contract'], ['history', 'History'], ...(s.god ? [['edit', 'Edit player']] : [])].map(([k, label]) => { const on = (s.ptab || 'overview') === k; return { label, go: () => gm.setState({ ptab: k }), color: on ? 'var(--color-accent-700)' : 'var(--color-text)', fw: on ? 600 : 400, ul: on ? 'var(--color-accent)' : 'transparent' }; });
    var ext = {};
    if (canExt) {
  ...
  const tm = !tmT ? {} : (() => { const tid = tmT.tid, ids = tid === 0 ? mine.slice() : s.rosters[tid].slice().sort((x, y) => P[y].ovr - P[x].ovr), pay = ids.reduce((a, id) => a + P[id].amt, 0), cs = T.filter(t => t.conf === tmT.conf).sort(byPct), ks = s.assets.filter(a => a.owner === tid && !usedPick(a));
    return { abbr: tmT.abbr, name: tmT.region + ' ' + tmT.name, line: ord(cs.indexOf(tmT) + 1) + ' in the ' + tmT.conf + ' · ' + tmT.div + ' Division', rec: tmT.w + '–' + tmT.l + ' · ' + strk(tmT) + ' · ' + l10(tmT) + ' last 10', market: (tmT.mkt >= 1.15 ? 'Large' : tmT.mkt >= .95 ? 'Mid-large' : tmT.mkt >= .85 ? 'Mid-size' : 'Small') + ' market',
      strat: tid === 0 ? 'Your team' : STRAT[strat[tid]][0], stratDesc: tid === 0 ? '' : STRAT[strat[tid]][1], payroll: money(pay), cap: pay > gm.TAX ? money(pay - gm.TAX) + ' over the tax' : pay > gm.CAP ? money(pay - gm.CAP) + ' over the cap' : money(gm.CAP - pay) + ' in cap space',
      ...
      rows: ids.map(id => ({ ...pBase(id), contract: money(P[id].amt) })), picks: ks.length ? ks.map(a => gm.pickLabel(a, T) + ' (' + projTxt(a).replace('Proj. ', '') + ')').join(' · ') : 'None', isOther: tid !== 0,
  ...
  const OA = { 'Win-Now Spender': 'Wants a contender now and will pay for it, up to the 2nd apron.', 'Frugal Profit-Seeker': 'Expects a profit every season and will not pay the luxury tax.', 'Asset Hoarder': 'Protects draft picks and young talent; hates giving up firsts.', 'Hype Focus': 'Cares about buzz: a full arena and a marquee star.', 'Meddling Micromanager': 'Second-guesses moves and insists his favorite player starts.' };
  const fav = mine.slice().sort((x, y) => P[y].pot - P[x].pot)[0], firsts = s.assets.filter(a => a.owner === 0 && a.rd === 1 && !usedPick(a)).length, bestOvr = Math.max(...mine.map(id => P[id].ovr)), full = att / 18800;
  const st3 = (ok, risk) => ok ? 'Met' : risk ? 'At risk' : 'Failing';
  const DEM = { 'Win-Now Spender': [['Make the playoffs (top 6, or through the play-in)', ord(seed) + ' in the ' + me.conf, st3(seed <= 6, seed <= 10)], ['Keep payroll under the 2nd apron', money(payroll), st3(payroll <= gm.AP2, false)]],
    'Frugal Profit-Seeker': [['Turn a profit of at least $10M', money(net), st3(net >= 10, net >= 0)], ['Stay under the luxury tax', money(payroll), st3(payroll <= gm.TAX, false)], ['Win at least 40% of games', fpct(me), st3(pct(me) >= .4, pct(me) >= .35)]],
    ...
    'Hype Focus': [['Fill 90% of the arena', Math.round(full * 100) + '%', st3(full >= .9, full >= .8)], ['Roster a star rated 65+', String(bestOvr), st3(bestOvr >= 65, bestOvr >= 62)]],
    'Meddling Micromanager': [['Start ' + P[fav].name, mine.indexOf(fav) < 5 ? 'Starting' : 'Bench', st3(mine.indexOf(fav) < 5, false)], ['Win at least half your games', fpct(me), st3(pct(me) >= .5, pct(me) >= .45)]] }[me.arch];
  const LIM = { 'Win-Now Spender': ['Payroll ceiling: ' + money(gm.AP2) + ' (2nd apron)', 'No profit requirement'], 'Frugal Profit-Seeker': ['Payroll ceiling: ' + money(gm.TAX) + ' (tax line)', 'Minimum profit: $10.0M'], 'Asset Hoarder': ['Payroll ceiling: ' + money(gm.AP1) + ' (1st apron)', 'Needs approval to trade any first-round pick'], 'Hype Focus': ['Payroll ceiling: ' + money(gm.AP1), 'Ticket price may not drop below $90'], 'Meddling Micromanager': ['Payroll ceiling: ' + money(gm.TAX), 'Signs off on every trade'] }[me.arch];
  const FIRE = { 'Win-Now Spender': ['Missing the playoffs two seasons in a row', 'Job security below 15'], 'Frugal Profit-Seeker': ['Losing money two seasons in a row', 'Paying the luxury tax', 'Job security below 15'], 'Asset Hoarder': ['Holding fewer than 2 firsts at season\u2019s end', 'Job security below 15'], 'Hype Focus': ['Attendance under 80% for a full season', 'Job security below 15'], 'Meddling Micromanager': ['Benching his favorite for 20+ games', 'Job security below 15'] }[me.arch];
  const secV = Math.round(cl(60 + (pct(me) - .5) * 80 + DEM.reduce((a, d) => a + (d[2] === 'Met' ? 6 : d[2] === 'At risk' ? -6 : -15), 0), 0, 100));
  const own = { name: me.owner, arch: me.arch, desc: OA[me.arch], sec: secV, secW: secV + '%', secLabel: secV >= 70 ? 'Secure' : secV >= 40 ? 'Stable' : secV >= 20 ? 'Warm seat' : 'Hot seat', secColor: secV >= 70 ? 'var(--gm-good)' : secV >= 40 ? 'var(--color-text)' : 'var(--gm-bad)',
    demands: DEM.map(([d0, cur, stt]) => ({ d: d0, cur, stt, color: stt === 'Met' ? 'var(--gm-good)' : stt === 'At risk' ? 'var(--color-accent-700)' : 'var(--gm-bad)' })), limits: LIM, fire: FIRE };
  ...
  const tacV = { groups: TAC.map(([k, label, opts, desc]) => ({ label, desc, opts: opts.map(o => ({ label: o, onClick: () => gm.setState(st => ({ tactics: { ...st.tactics, [k]: o } })), color: s.tactics[k] === o ? 'var(--color-accent-700)' : 'var(--color-text)', ring: s.tactics[k] === o ? 'inset 0 0 0 1px var(--color-accent)' : 'none' })) })), fit: (() => { const f = gm.tacFit(mine, s.tactics); return (f >= 0 ? '+' : '−') + Math.abs(f).toFixed(1) + ' team strength from how these settings fit your roster'; })(),
    total: Math.round(mine.reduce((a, id) => a + (P[id].inj || P[id].dev ? 0 : P[id].min), 0)), rot: mine.map((id, i) => ({ name: P[id].name, pos: P[id].pos, tag: P[id].inj ? 'Injured' : P[id].dev ? 'Dev league' : i < 5 ? 'Starter' : '', v: Math.round(P[id].min), val: Math.round(P[id].min) + ' min', dis: !!(P[id].inj || P[id].dev), set: e => { P[id].min = +e.target.value; gm.setState(st => ({ gv: (st.gv || 0) + 1 })); } })) };
  ...
  const gp = me.w + me.l, PH = [['regular', 'Regular season'], ['playin', 'Play-in'], ['playoffs', 'Playoffs'], ['lottery', 'Lottery'], ['draft', 'Draft'], ['fa', 'Free agency'], ['preseason', 'Preseason']];
  const phK = s.phase === 'playoffs' && s.po && s.po.champ != null ? 'playoffs' : s.phase;
  ...
  if (s.phase === 'regular') { if (gp < 82) { ph.note = 'Game ' + gp + ' of 82'; ph.actions = [act('Play a day', () => gm.sim(1)), act('Play a week', () => gm.sim(7)), act('Sim to end of regular season', () => gm.sim(82))]; } else { ph.note = 'Regular season complete'; ph.actions = [act('Start the play-in', () => gm.startPlayin(), true)]; } }
  else if (s.phase === 'playoffs') { if (s.po.champ == null) { ph.note = 'Best-of-7 series'; ph.actions = [act('Sim a game', () => gm.simPo('game')), act('Sim round', () => gm.simPo('round')), act('Sim to champion', () => gm.simPo('all'), true)]; } else { ph.note = T[s.po.champ].region + ' ' + T[s.po.champ].name + ' are champions'; ph.actions = [act('Go to the draft lottery', () => gm.setState({ phase: 'lottery', screen: 'playoffs' }), true)]; } }
  ...
  else if (s.phase === 'draft') { ph.note = draftDone ? 'Draft complete' : 'Pick ' + (s.pi + 1) + ' of ' + s.picks.length; { const c = s.picks[s.pi], mineNow = c && gm.owner2027(c.orig, s.assets) === 0; if (!draftDone && mineNow) ph.note = 'You are on the clock: pick from the board'; ph.actions = draftDone ? [act('Open free agency', () => gm.startFA(), true)] : mineNow ? [act('Go to the board', go('draft')), act('Auto-pick for me', () => gm.aiDraft(false))] : [act('Sim to my pick', () => gm.aiDraft(true), true), act('Auto-draft the rest', () => gm.aiDraft(false))]; } }
  ...
  else if (s.phase === 'preseason') { const over = mine.length > 15; ph.note = over ? 'Cut to 15 players before opening night (' + mine.length + ' now)' : 'Players have developed; ratings updated'; ph.actions = [{ ...act('Start the regular season', () => gm.startSeason(), true), dis: over }]; }
  ph.actions = ph.actions.map(a => ({ ...a, dis: !!a.dis }));
  { const prim = ph.actions.find(a => a.cls === 'btn-primary' && !a.dis), NEXT = { regular: 'playin', playoffs: s.po && s.po.champ != null ? 'lottery' : null, lottery: 'draft', draft: 'fa', fa: 'preseason', preseason: 'regular' }[s.phase], VIEW = { regular: 'dash', playin: 'playoffs', playoffs: 'playoffs', lottery: 'playoffs', draft: 'draft', fa: 'fa', preseason: 'dash' };
  ...
  const serRow = x => { const done = x.wa === 4 || x.wb === 4, aw = x.wa === 4, bw = x.wb === 4; return { conf: x.conf, na: '(' + x.sa + ') ' + T[x.a].region + ' ' + T[x.a].name, nb: '(' + x.sb + ') ' + T[x.b].region + ' ' + T[x.b].name, wa: x.wa, wb: x.wb, fa: aw ? 600 : 400, fb: bw ? 600 : 400, ca: x.a === 0 ? 'var(--color-accent-700)' : done && !aw ? 'var(--color-neutral-600)' : 'var(--color-text)', cb: x.b === 0 ? 'var(--color-accent-700)' : done && !bw ? 'var(--color-neutral-600)' : 'var(--color-text)', oa: openTeam(x.a), ob: openTeam(x.b) }; };
  let poRounds;
  ...
  const pov = { rounds: poRounds, hasChamp: !!(s.po && s.po.champ != null), champName: s.po && s.po.champ != null ? T[s.po.champ].region + ' ' + T[s.po.champ].name : '', champSub: s.po && s.po.champ != null ? 'Beat the ' + T[s.po.runner].region + ' ' + T[s.po.runner].name + ' in the Finals' : '', seasonLbl: gm.seasonLbl(),
    ...
    lotto: (s.lotto || []).slice(0, 14).map(x => ({ n: x.n, name: T[x.t].region + ' ' + T[x.t].name, move: x.from > x.n ? '▲ from ' + x.from : x.from < x.n ? '▼ from ' + x.from : '—', color: x.t === 0 ? 'var(--color-accent-700)' : 'var(--color-text)', open: openTeam(x.t) })), hasLotto: !!s.lotto };
  ...
  const titles = { tactics: 'Tactics & rotation', scouting: 'Global scouting', overseas: 'Overseas market', dev: 'Player development', owner: 'Ownership', playoffs: 'Playoffs', settings: 'Settings', game: 'Live game', schedule: 'Schedule', tx: 'League transactions', short: 'Shortlist', dash: 'Dashboard', standings: 'Standings', roster: 'Roster', depth: 'Roster construction', player: 'Player', fin: 'Finances', trade: 'Trade', fa: 'Free agency', draft: 'Draft' };
  const metas = { tactics: 'You are GM and head coach', scouting: s.scouts.length + ' scouts · agent reputation ' + s.agentRep, overseas: (s.overseas || []).length + ' players abroad', dev: 'Growth is calculated monthly', owner: me.owner + ' · ' + me.arch, playoffs: gm.seasonLbl() + ' postseason', settings: '', game: next.oppName, schedule: (me.w + me.l) + ' played · ' + (82 - me.w - me.l) + ' remaining', tx: 'All 30 teams', short: s.lists.reduce((a, l) => a + l.ids.length, 0) + ' players tracked', dash: '', standings: (me.w + me.l) + ' of 82 games played', roster: mine.length + ' players · payroll ' + money(payroll), depth: 'From your current rotation', player: '', fin: 'Market size: ' + (mk >= 1.15 ? 'large' : mk >= .95 ? 'mid-large' : mk >= .85 ? 'mid' : 'small') + ' (' + ord(mRank) + ' of 30) · cap ' + money(gm.CAP), trade: 'Payroll ' + money(payroll), fa: s.fa.length + ' available · ' + (15 - mine.length) + ' roster spot' + (15 - mine.length === 1 ? '' : 's'), draft: isCur ? 'Big board · ' + clsIds.length + ' prospects left' : 'Class of ' + s.dClass + ' · ' + clsIds.length + ' tracked' };
  const page = { kicker: variant === 'B' ? 'Baltimore Tides' : dateLong, title: titles[s.screen], meta: metas[s.screen] };
  ...
  const dlg = !dg ? {} : dg.type === 'abroad' ? { title: 'Release ' + dp.name + ' to play overseas?', body: 'He joins a club abroad, where heavy minutes can rebuild his game. He stays on the overseas market and can be signed back later.', cta: 'Release overseas', confirm: () => gm.confirmDialog() } : dg.type === 'sign' && dp.abroad ? { title: 'Sign ' + dp.name + ' from ' + dp.abroad.club + '?', body: money(gm.askFor(dp, s)) + ' per year. His ' + dp.abroad.clause.toLowerCase() + ' costs ' + money(dp.abroad.fee) + (dp.abroad.fee > .85 ? ', and ' + money(dp.abroad.fee - .85) + ' of it counts against the cap' : '') + '. Expect a 15-game adjustment period.', cta: 'Sign player', confirm: () => gm.confirmDialog() } : dg.type === 'sign' ? { title: 'Sign ' + dp.name + '?', body: money(gm.askFor(dp, s)) + ' per year through ' + dp.exp + (how ? ', using ' + (how === 'Cap space' ? 'cap space' : how === 'Minimum' ? 'the veteran minimum' : 'your mid-level exception') : '') + '. Payroll after: ' + money(payroll + gm.askFor(dp, s)) + '.', cta: 'Sign player', confirm: () => gm.confirmDialog() } : { title: 'Release ' + dp.name + '?', body: 'He goes to free agency and any team can sign him. His ' + money(dp.amt) + ' comes off your books.', cta: 'Release', confirm: () => gm.confirmDialog() };
  ...
  return {
    ...
    play1: () => { if (s.phase === 'regular' && me.w + me.l < 82) gm.setState({ screen: 'game', q: '' }); }, quick1: () => gm.sim(1), play7: () => gm.sim(7), live, schedRows, txRows, txSegs, shortlists, noLists: s.lists.length === 0, newList: s.newList, noNewList: !s.newList.trim(),
    ...
    autoLineup: () => gm.setState(st => ({ rosters: { ...st.rosters, 0: st.rosters[0].slice().sort((x, y) => P[y].ovr - P[x].ovr) }, sort: { ...st.sort, roster: ['rk', 1] } })),
    ...
    log: s.log, noLog: s.log.length === 0,
    lm, hasList: !!s.listModal, closeList: () => gm.setState({ listModal: null }), scoutRegions, scoutsV, promisesV, noPromises: promisesV.length === 0, repV, ovRows, tacV, natRows, resetNat: () => gm.setState({ natW: natDefault() }), devRows, reportsV, noReports: s.reports.length === 0, own, god, isGod: !!s.god, ph, pov, settings, hasProg, progRows, hasTeamModal: !!tmT, tm, closeTeam: () => gm.setState({ teamModal: null }), viewTradeTeam: openTeam(s.tTid), themeLabel: dark ? 'Light mode' : 'Dark mode', toggleTheme: () => gm.setState({ theme: dark ? 'light' : 'dark' }), rootRef, hasModal: !!s.modal, closeModal: () => gm.setState({ modal: false }), ptabs, ext, hasDialog: !!dg, dlg, closeDialog: () => gm.setState({ dialog: null }), stop: e => e.stopPropagation()
  };
}
