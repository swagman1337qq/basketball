...
export function buildView(gm: Game, rootRef: RefObject<HTMLDivElement | null>, extra: ViewExtras): VM {
  ...
  const g = gm.userGame(s.day), opp = T[g.opp];
  const next = { logo: logo(g.opp, 40), myLogo: logo(0, 40), smLogo: logo(g.opp, 22), openT: openTeam(g.opp), when: gm.fmtS(s.day), vs: g.home ? 'vs' : 'at', abbr: opp.abbr, oppName: opp.region + ' ' + opp.name, oppRec: opp.w + '–' + opp.l, myRec: me.w + '–' + me.l, line: (g.home ? 'Home · ' + me.region : 'Road · ' + opp.region) + ' · ' + ord(T.filter(t => t.conf === opp.conf).sort(byPct).indexOf(opp) + 1) + ' in the ' + opp.conf };
  const results = s.results.slice(0, 6).map(r => ({ logo: logo(r.opp, 16), openT: openTeam(r.opp), date: gm.fmtS(r.day), opp: (r.home ? 'vs ' : 'at ') + T[r.opp].abbr, wl: r.win ? 'W' : 'L', color: r.win ? 'var(--color-accent-700)' : 'var(--color-neutral-600)', score: r.us + '–' + r.them }));
  ...
  const confMini = confT.slice(0, 10).map((t, i) => ({ logo: logo(t.tid, 18), seed: i + 1, name: t.region + ' ' + t.name, rec: t.w + '–' + t.l, gb: gb(t, confT[0]), bg: t.tid === 0 ? 'var(--color-accent-100)' : 'transparent', fw: t.tid === 0 ? 600 : 400, openT: openTeam(t.tid), line: i === 5 ? '1px solid var(--color-text)' : '1px solid var(--color-divider)' }));
  const dashStats = [
  ...
  const dr = { isCurrent: isCur, isFuture: !isCur, hasAdvice: advice.length > 0, advice, noAdvice: !myNext,
    ...
    order: s.picks.map((x, i) => { const ow = gm.owner2027(x.orig, s.assets); return { n: x.n, logo: logo(ow, 16), openT: openTeam(ow), abbr: T[ow].abbr, who: x.pid ? P[x.pid].name : i === s.pi ? 'On the clock' : (ow !== x.orig ? 'via ' + T[x.orig].abbr : ''), fs: x.pid ? 'normal' : 'italic', bg: i === s.pi ? 'var(--color-accent-100)' : 'transparent', color: ow === 0 ? 'var(--color-accent-700)' : 'var(--color-text)', fw: ow === 0 ? 600 : 400 }; }) };
  ...
  const live = { home: g.home ? usL : themL, away: g.home ? themL : usL, userSide: g.home ? 'home' : 'away', tactics: s.tactics, logos: { home: logo(hT, 44), away: logo(aT, 44), homeSm: logo(hT, 22), awaySm: logo(aT, 22) }, onPlayer: id => gm.setState({ pid: id, modal: true, ptab: 'overview', extYears: null, extAmt: null, extMsg: null }), onTeam: tid => gm.setState({ teamModal: tid }), onFinish: r => { gm.sim(1, r); gm.setState({ screen: 'dash' }); } };
  const left = s.phase === 'regular' ? Math.max(0, 82 - me.w - me.l) : 0;
  const schedRows = [...s.results.slice().reverse().map(r => ({ logo: logo(r.opp), openT: openTeam(r.opp), date: gm.fmtS(r.day), opp: (r.home ? 'vs ' : 'at ') + T[r.opp].region + ' ' + T[r.opp].name, rec: T[r.opp].w + '–' + T[r.opp].l, res: (r.win ? 'W ' : 'L ') + r.us + '–' + r.them, resColor: r.win ? 'var(--color-accent-700)' : 'var(--color-neutral-600)', isNext: false, notNext: true, bg: 'transparent' })),
  ...
  const serRow = x => { const done = x.wa === 4 || x.wb === 4, aw = x.wa === 4, bw = x.wb === 4; return { la: logo(x.a, 16), lb: logo(x.b, 16), conf: x.conf, na: '(' + x.sa + ') ' + T[x.a].region + ' ' + T[x.a].name, nb: '(' + x.sb + ') ' + T[x.b].region + ' ' + T[x.b].name, wa: x.wa, wb: x.wb, fa: aw ? 600 : 400, fb: bw ? 600 : 400, ca: x.a === 0 ? 'var(--color-accent-700)' : done && !aw ? 'var(--color-neutral-600)' : 'var(--color-text)', cb: x.b === 0 ? 'var(--color-accent-700)' : done && !bw ? 'var(--color-neutral-600)' : 'var(--color-text)', oa: openTeam(x.a), ob: openTeam(x.b) }; };
  let poRounds;
  ...
  const pov = { rounds: poRounds, hasChamp: !!(s.po && s.po.champ != null), champName: s.po && s.po.champ != null ? T[s.po.champ].region + ' ' + T[s.po.champ].name : '', champSub: s.po && s.po.champ != null ? 'Beat the ' + T[s.po.runner].region + ' ' + T[s.po.runner].name + ' in the Finals' : '', seasonLbl: gm.seasonLbl(),
    ...
    lotto: (s.lotto || []).slice(0, 14).map(x => ({ logo: logo(x.t, 16), n: x.n, name: T[x.t].region + ' ' + T[x.t].name, move: x.from > x.n ? '▲ from ' + x.from : x.from < x.n ? '▼ from ' + x.from : '—', color: x.t === 0 ? 'var(--color-accent-700)' : 'var(--color-text)', open: openTeam(x.t) })), hasLotto: !!s.lotto };
  ...
  const metas = { tactics: 'You are GM and head coach', scouting: s.scouts.length + ' scouts · agent reputation ' + s.agentRep, overseas: (s.overseas || []).length + ' players abroad', dev: 'Growth is calculated monthly', owner: me.owner + ' · ' + me.arch, playoffs: gm.seasonLbl() + ' postseason', settings: '', game: next.oppName, schedule: (me.w + me.l) + ' played · ' + (82 - me.w - me.l) + ' remaining', tx: 'All ' + T.length + ' teams', short: s.lists.reduce((a, l) => a + l.ids.length, 0) + ' players tracked', dash: '', standings: (me.w + me.l) + ' of 82 games played', roster: mine.length + ' players · payroll ' + money(payroll), depth: 'From your current rotation', player: '', fin: 'Market size: ' + (mk >= 1.15 ? 'large' : mk >= .95 ? 'mid-large' : mk >= .85 ? 'mid' : 'small') + ' (' + ord(mRank) + ' of ' + T.length + ') · cap ' + money(gm.CAP), trade: 'Payroll ' + money(payroll), fa: s.fa.length + ' available · ' + (15 - mine.length) + ' roster spot' + (15 - mine.length === 1 ? '' : 's'), draft: isCur ? 'Big board · ' + clsIds.length + ' prospects left' : 'Class of ' + s.dClass + ' · ' + clsIds.length + ' tracked' };
  ...
  return {
    myName, myRegion: me.region, myAbbr: me.abbr, myLogo: logo(0, 44), myLogoLg: logo(0, 56), myLogoSm: logo(0, 34), theirLogo: logo(s.tTid, 28), myLogoTr: logo(0, 28),
    ...
    play1: () => { if (s.phase === 'regular' && me.w + me.l < 82 && !s.simming) gm.setState({ screen: 'game', q: '' }); }, quick1: () => gm.sim(1), play7: () => gm.sim(7), live, schedRows, txRows, txSegs, shortlists, noLists: s.lists.length === 0, newList: s.newList, noNewList: !s.newList.trim(),
