...
export function financesOf(g: Game, s: any, tid: number) {
  ...
  const exp: [string, number][] = [['Player payroll', payroll], ...(taxBill ? [[repeater ? 'Luxury tax (repeater rates)' : 'Luxury tax', taxBill] as [string, number]] : []), ['Arena & game operations', 55.0], ['Front office & staff', 25.0], ['Team travel', 9.0], ...(share > 0 ? [['Revenue sharing paid', share] as [string, number]] : []), ...(club?.buyoutCash ? [['Overseas buyouts', club.buyoutCash] as [string, number]] : []), ...(club?.bonusPaid ? [['Incentive bonuses paid', club.bonusPaid] as [string, number]] : []), ['Coaching', b.Coaching], ['Health', b.Health], ['Facilities', b.Facilities], ['Scouting', b.Scouting]];
...
export function seasonReview(g: Game) {
  g.setState(s => {
    ...
    s.managed.forEach(tid => {
      const rv = ownerReview(g, s, tid), fin = finOf(tid), reasons = s.ownerFiring === false ? [] : fireReasons(g, s, tid, rv, fin);
